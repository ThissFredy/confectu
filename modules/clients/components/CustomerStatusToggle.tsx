...
export function CustomerStatusToggle({
  ...
  async function handleAction(formData: FormData) {
    ...
    if (response.success) {
      showToast.success(
        customer.isActive ? "Cliente desactivado" : "Cliente reactivado",
        { position: "top-right" },
      );
      setStep(null);
      router.refresh();
  ...
  const actionLabel = customer.isActive ? "desactivar" : "reactivar";
  const actionNoun = customer.isActive ? "desactivación" : "reactivación";
  const buttonLabel = customer.isActive ? "Desactivar" : "Reactivar";
  const buttonVariant = customer.isActive ? "danger" : "primary";
    ...
    <div>
      {step === null ? (
        <button
          type="button"
          onClick={() => setStep(1)}
          aria-label={`${buttonLabel} cliente ${customer.name}`}
        ...
        <div
          role="dialog"
          aria-modal="true"
          aria-labelledby="customer-toggle-title"
          className="rounded-lg border border-zinc-200 p-4 dark:border-zinc-800"
        >
          <p
            id="customer-toggle-title"
            className="text-sm text-zinc-700 dark:text-zinc-300"
          >
            {step === 1
              ? `¿Estás seguro de ${actionLabel} este cliente?`
